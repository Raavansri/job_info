# with open("GeeksforGeeks.html","r") as files:
#     content = files.read()
#     print(content)
...
def scrape ():
    ...
    #     fi.write(getreq.text)
    #soup = BeautifulSoup(getreq.content,"html.parser")

    soup = BeautifulSoup(getreq.content,'lxml')    # lxml is parser for make html tags as pyhton objects
    tags = soup.find_all('li',class_="clearfix job-bx wht-shd-bx")     # it filter div tags with calss name item
    for tag in tags:
        ...
        if 'few' in post:
            ...
            loc = tag.find('li',class_="srp-walkin-dtl").span.text.replace(" ","")  # data from span and replace sapce
            link = tag.header.h2.a['href']                    # fetch data from header in h2 in a and in a href link we took
...
#for refr in tags:
    #words=refr.h3.text.split()[-1]       # the collected tags filltered with .a tags text .split can split words to process -1 ia last word indwx
